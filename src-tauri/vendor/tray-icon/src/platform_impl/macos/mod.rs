...
fn on_tray_click(this: &TrayTarget, button: MouseButton) {
    ...
    unsafe {
        ...
        if button == MouseButton::Right || (menu_on_left_click && button == MouseButton::Left) {
            // Retain the menu out of the RefCell: performClick runs a nested
            // event loop that may re-enter set_menu (upstream PR #365).
            let menu = this.ivars().menu.borrow().clone();
            let has_items = menu.as_ref().is_some_and(|menu| menu.numberOfItems() > 0);
            if has_items {
                this.ivars().status_item.setMenu(menu.as_deref());
                ns_button.performClick(None);
                this.ivars().status_item.setMenu(None);
            } else {
